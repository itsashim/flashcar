...
@section('content')
            ...
            <div class="overlay" id="myNav">
                <div class="left-sidebar-categories order-1 overlay-content">
                    ...
                    <div class="viewBox listBox new-listBox">
                        <ul class="list m-0 " >
                            @if (isset($category) && $category->slug == 'medical-appliances')
                                @if (count($subCategories) > 0)
                                    <ul class="sublist">
                                        @foreach ($subCategories as $subCategory)
                                            <li class="inactive">
                                                <a href="{{ asset('sub-categories/' . $subCategory->slug) }}"
                                                    @if (isset($sCategory) && $sCategory->id == $subCategory->id) class="text-green" @else class="text-black" @endif>
                                                    {{ $subCategory->name }} 
                                                </a>
                                            </li>
                                        @endforeach
                            ...
                            @else
                                @foreach ($categories as $c)
                                    @if (isset($category->id) && $c->id == $category->id)
                                        <li class="active last">
                                            <a href="{{ asset('categories/' . $c->slug) }}" class="text-green">
                                                {{ $c->name }} </a>
                                            @if (count($subCategories) > 0)
                                                <ul class="sublist">
                                                    @foreach ($subCategories as $subCategory)
                                    ...
                                    @else
                                        <li class="inactive"><a class="text-black"
                                                href="{{ asset('categories/' . $c->slug) }}"> {{ $c->name }} </a></li>
                                    @endif
                                @endforeach
                            @endif
                        </ul>
    ...
    -->
            <!--                   <li class="inactive">-->
            <!--                     <a href="{{ asset('sub-categories/' . $subCategory->slug) }}" @if (isset($sCategory) && $sCategory->id == $subCategory->id) class="text-green" @else class="text-black" @endif > {{ $subCategory->name }} </a>-->
            <!--                   </li>-->
            <!--
    @endforeach-->
            <!--               </ul>-->
    ...
    -->
            <!--             <li class="active last">-->
            <!--               <a href="{{ asset('categories/' . $c->slug) }}" class="text-green"  > {{ $c->name }} </a>-->
            <!--               @if (count($subCategories) > 0)
    -->
            <!--               <ul class="sublist">-->
            <!--                   @foreach ($subCategories as $subCategory)
    ...
    @endif-->
        ...
        <!--             @else-->
            <!--               <li class="inactive"><a class="text-black" href="{{ asset('categories/' . $c->slug) }}"> {{ $c->name }} </a></li>-->
            <!--
    @endif-->
    ...
    @endforeach-->
            ...
            <!--<span class="open-category" style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776; open</span>-->
                                    ...
                                    <ul class="list m-0">
                                        @if (isset($category) && $category->slug == 'medical-appliances')
                                            @if (count($subCategories) > 0)
                                                <ul class="sublist">
                                                    @foreach ($subCategories as $subCategory)
                                                        <li class="inactive">
                                                            <a style="color: #333" href="{{ asset('sub-categories/' . $subCategory->slug) }}"
                                                                @if (isset($sCategory) && $sCategory->id == $subCategory->id) class="text-green" @else class="text-black" @endif>
                                                                {{ $subCategory->name }} </a>
                                                        </li>
                                                    @endforeach
                                        ...
                                        @else
                                            @foreach ($categories as $c)
                                                @if (isset($category->id) && $c->id == $category->id)
                                                    <li class="active last">
                                                        <a style="color: #333" href="{{ asset('categories/' . $c->slug) }}" class="text-green">
                                                            {{ $c->name }} </a>
                                                        @if (count($subCategories) > 0)
                                                            <ul class="sublist">
                                                                @foreach ($subCategories as $subCategory)
                                                ...
                                                @else
                                                    <li class="inactive"><a style="color: #333"
                                                            href="{{ asset('categories/' . $c->slug) }}"> {{ $c->name }} </a>
                                                    </li>
                                                @endif
                                            @endforeach
                                        @endif
                ...
                <div class="col-lg-9 col-md-7 col-sm-12 ">
                            <h2 class="mb-4" style="color:#555">Accesorieslist List</h2>
                                    ...
                                    @foreach ($accessories as $product)
                                        <div class="mb-2 col-12 col-sm-6 col-md-6 col-lg-4">
                                            <a title="Show products in category Orthopaedics"
                                                href="{{ asset('product_details/' . $product->slug) }}" target="_self"
                                                class="">
                                                    ...
                                                    <div class="card-body">
                                                        <a href="{{ asset('product_details/' . $product->slug) }}">
                                                            <button data-id="{{ $product->id }}"
                                                                ...
                                                                to cart</button>
                                                            <h5 style="font-weight: 600;font-size: 1rem;color: #333">{{ $product->name }}</h5>
                                                        </a>
                                                        @if ($product->offer_price)
                                                            <span style="text-decoration:line-through;color:red;display: block">Rs
                                                                {{ $product->price }}</span> <span
                                                                style="font-size:20px;color:green;">Rs. {{ $product->offer_price }}
                                                            </span>
                                                        @else
