...
class ApiController extends Controller
{
    ...
    {
        $subscriptions = Subscription::with('packagedata')->where("user_id", $request->get("user_id"))->orderby("id", "DESC")->paginate(10);
        return response(['status'=>true,'data'=>$subscriptions]);
    }



    public function mysubscriptionOld(Request $request)
    {
        $response = array("status" => 0, "msg" => "Validation error");
        $rules = [
            'user_id' => 'required'
