...
class AppointmentController extends Controller
{
     ...
     public function hospitalAppointment(Request $request){
          $appointments = Appointment::when($request->start_date && $request->end_date,function($q)use($request){
                              $q->whereBetween('date',[$request->start_date,$request->end_date]);
                         })
                         ->when(auth()->user()->usertype == 3,function($q){
                              $doctor = Doctor::where('user_id', auth()->user()->id)->first();
                              $q->where('doc_id', $doctor->id);
                         })
                         ->when(auth()->user()->usertype == 4,function($q){
     ...
     public function doctorAppointment(Request $request){
          $appointments = Appointment::when($request->start_date && $request->end_date,function($q)use($request){
            ...
            foreach($appointments as $app)
            {
                ...
                $hospital_name= $app->hospital->name;
            }
          return view('admin.appointment.index',compact('appointments','gender','doctor_name','patient_name','hospital_name'));
     }

     public function showappointment($startdate, $enddate)
     {
          if ($startdate == 0 && $enddate == 0) {
               $data = Appointment::with('doctors', 'services', 'department');
          } else {
               $data = Appointment::with('doctors', 'services', 'department')->whereBetween('date', [$startdate, $enddate]);
          }

          if (auth()->user()->usertype == 3) {
               $doctor = Doctor::where('user_id', auth()->user()->id)->first();
               $data = $data->where('doc_id', $doctor->id);
          }
          if (auth()->user()->usertype == 4) {
               $hospital = Hospital::where('user_id', auth()->user()->id)->first();
               $data = $data->where('hospital_id', $hospital->id);
          }
          $data = $data->get();
     ...
     public function update(Request $request)
     {
          try {
               DB::beginTransaction();
               $msg = "";
               $store = Appointment::find($request->app_id);
               $setting = Setting::find(1);
               $user = User::find($store->user_id);
               $staus_id = $request->status;
               if (!$user) {
                    Session::flash('message', __('messages.user_not_exist'));
                    Session::flash('alert-class', 'alert-success');
                    return redirect()->back();
               ...
               if ($staus_id == 3) { //approve
                    $msg = __('messages.approve appointement successfully');
                    $android = $this->send_notification_android($setting->android_server_key, $store->user_id, $msg, $store->id);
                    $ios = $this->send_notification_IOS($setting->ios_server_key, $store->user_id, $msg, $store->id);
               }
               if ($staus_id == 6) { //reject
                    $msg = __('messages.appointement reject successfully');
                    $android = $this->send_notification_android($setting->android_server_key, $store->user_id, $msg, $store->id);
     ...
     {
          $appointment = Appointment::with('doctors', 'services', 'department', 'patient', 'hospital')->whereId($id)->firstOrFail();
